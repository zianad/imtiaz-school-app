...
import { Announcement, Teacher } from '../../types';
...
const PrincipalAnnouncements: React.FC<PrincipalAnnouncementsProps> = ({ announcements, teachers, onAddAnnouncement, onBack, onLogout }) => {
    ...
    const handleSave = async () => {
        ...
        if (targetAudience === 'teachers') {
            // If not all teachers are selected, we specify the IDs.
            // If all are selected, we can omit the `teacherIds` field, which means "all teachers".
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-xl border-t-8 border-blue-600 w-full relative">
            ...
            <div className="mb-8 p-4 bg-gray-50 rounded-lg shadow-inner space-y-4">
                <textarea value={content} onChange={e => setContent(e.target.value)} placeholder={t('announcementContent')} rows={5} className="w-full p-3 border-2 rounded-lg" />
                ...
                <div className="space-y-2">
                    <label className="font-semibold block">{t('targetAudience')}:</label>
                ...
                {targetAudience === 'teachers' && (
                    <div className="mt-4 p-3 bg-gray-100 rounded-lg border-2 border-dashed">
                        <h3 className="font-semibold mb-2 text-gray-700">{t('selectTeachers')}:</h3>
                        <div className="flex items-center mb-2 pb-2 border-b">
                            ...
                            />
                            <label htmlFor="select-all-teachers" className="mr-2 font-bold text-gray-800">{t('all')}</label>
                        </div>
                        <div className="max-h-32 overflow-y-auto grid grid-cols-2 gap-2">
                            {teachers.map(teacher => (
                                <div key={teacher.id} className="flex items-center p-1 bg-white rounded-md">
                                     <input
                                        type="checkbox"
                                    ...
                                    />
                                    <label htmlFor={`teacher-${teacher.id}`} className="mr-2 text-sm text-gray-700">{teacher.name}</label>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <div>
                <h2 className="text-xl font-semibold text-gray-700 mb-3 text-center border-t pt-4">{t('sentAnnouncements')}</h2>
                <div className="max-h-60 overflow-y-auto space-y-2 p-2">
                    {announcements.length > 0 ? announcements.map(ann => (
                        <div key={ann.id} className="bg-white p-3 rounded-lg shadow-sm">
                            ...
                            <p className="text-xs text-gray-500">To: {t(ann.targetAudience)}</p>
                        </div>
                    )) : <p className="text-center text-gray-500">{t('noAnnouncements')}</p>}
                </div>
            </div>
