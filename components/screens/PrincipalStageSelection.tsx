...
import { School, EducationalStage } from '../../core/types';
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl border-t-8 border-gray-800 dark:border-gray-600 animate-fade-in text-center w-full relative">
            ...
            <div className="text-center mb-8">
                {school.logoUrl ? (
                    <img src={school.logoUrl} alt={`${school.name} Logo`} className="w-24 h-24 rounded-full object-contain mb-4 mx-auto shadow-md" />
                ) : null}
                <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-2">{school.name}</h1>
                <p className="text-lg text-gray-600 dark:text-gray-300">{t('selectStage')}</p>
            </div>
