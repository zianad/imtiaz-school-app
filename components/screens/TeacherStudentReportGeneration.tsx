...
const TeacherStudentReportGeneration: React.FC<TeacherStudentReportGenerationProps> = ({ student, subject, onGenerateComment, onSendForReview, onBack, onLogout }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if(comment.startsWith('Not enough grades')) { // Check for specific error message
                setError(comment);
            } else {
    ...
    const getGradeSummary = () => {
        ...
        if (!grades || grades.every(g => g.score === null)) {
            return <p className="text-center text-gray-500">لا توجد نقاط لعرضها.</p>;
        }
        ...
        const validSubSubjects = SUBJECT_MAP[subject].map(subSubject => {
            ...
            return (
                <li key={subSubject} className="flex justify-between items-center bg-gray-100 p-2 rounded-md">
                    <span className="font-semibold text-gray-700">{subSubject}</span>
    ...
    return (
        <div className="bg-white p-6 md:p-8 rounded-2xl shadow-xl border-t-8 border-blue-600 animate-fade-in relative">
            ...
            <h1 className="text-2xl font-bold text-gray-800 mb-2 text-center">ملاحظة التلميذ(ة)</h1>
            ...
            <div className="bg-blue-50 border-2 border-dashed border-blue-200 rounded-lg p-4 mb-4">
                <h3 className="font-bold text-gray-700 mb-2 text-center">ملخص النقاط</h3>
            ...
            {generatedComment && (
                <div className="mt-6 animate-fade-in">
                    <textarea
                        ...
                        rows={8}
                        className="w-full p-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition bg-gray-50"
            ...
            {sendSuccess && <p className="text-green-600 text-center font-semibold mt-2 animate-pulse">{sendSuccess}</p>}
