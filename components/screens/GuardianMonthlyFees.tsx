...
import { Student, MonthlyFeePayment, School } from '../../types';
...
const GuardianMonthlyFees: React.FC<GuardianMonthlyFeesProps> = ({ student, school, payments, onPay, onBack, onLogout }) => {
                ...
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 relative">
                    {paymentSuccess ? (
                        <div className="text-center p-4">
                            ...
                            <p className="text-gray-600 mt-2">شكراً لكم!</p>
                        </div>
                    ) : (
                        <form onSubmit={handlePaymentSubmit}>
                            <h2 className="text-xl font-bold text-gray-800 text-center mb-2">{t('payByCard')}</h2>
                            <p className="text-center text-gray-600 mb-4">{selectedMonth.name} {selectedMonth.year}</p>
                            ...
                            <div className="mb-6 p-4 bg-gray-50 rounded-lg space-y-3">
                                ...
                                <label className="flex justify-between items-center text-gray-800 cursor-pointer">
                            ...
                            <div className="space-y-4">
                                <input type="tel" value={cardInfo.number} onChange={e => setCardInfo({...cardInfo, number: e.target.value.replace(/\D/g, '')})} placeholder={t('cardNumber')} maxLength={16} className="w-full p-3 border-2 bg-gray-100 text-black border-gray-300 rounded-lg text-center tracking-wider" required />
                                <div className="flex gap-4">
                                    <input type="tel" value={cardInfo.expiry} onChange={e => setCardInfo({...cardInfo, expiry: e.target.value})} placeholder={t('expiryDate')} className="w-1/2 p-3 border-2 bg-gray-100 text-black border-gray-300 rounded-lg text-center" required />
                                    <input type="tel" value={cardInfo.cvc} onChange={e => setCardInfo({...cardInfo, cvc: e.target.value.replace(/\D/g, '')})} placeholder={t('cvc')} maxLength={3} className="w-1/2 p-3 border-2 bg-gray-100 text-black border-gray-300 rounded-lg text-center" required />
                                </div>
                                <input type="text" value={cardInfo.name} onChange={e => setCardInfo({...cardInfo, name: e.target.value})} placeholder={t('cardholderName')} className="w-full p-3 border-2 bg-gray-100 text-black border-gray-300 rounded-lg text-center" required />
                            </div>
                            ...
                            <div className="mt-6 space-y-2">
                                ...
                                <button type="button" onClick={handleCloseModal} disabled={isProcessing} className="w-full bg-gray-200 text-gray-700 font-bold py-2 rounded-lg hover:bg-gray-300 transition">
                                    {t('cancel')}
                                </button>
                            </div>
                        </form>
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-xl border-t-8 border-blue-600 w-full relative">
            ...
            <p className="text-center text-gray-600 mb-6">{t('monthlyFeePrompt')}</p>
