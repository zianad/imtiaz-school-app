...
import { School, Page, SchoolFeature, EducationalStage, Principal } from '../../types';
...
interface SuperAdminSchoolManagementProps {
    ...
    onDeletePrincipal: (stage: EducationalStage, principalId: string) => void;
...
const SuperAdminSchoolManagement: React.FC<SuperAdminSchoolManagementProps> = ({
    ...
    const teacherFeatures: SchoolFeature[] = [
        ...
        'teacherManageProjects', 'teacherManageLibrary', 'teacherManageAlbum'
    ];
    ...
    const guardianFeatures: SchoolFeature[] = [
        ...
        'guardianViewAlbum', 'guardianPayFees', 'guardianSubmitComplaints', 'guardianRequestInterview'
    ];


    const allStages = [EducationalStage.PRIMARY, EducationalStage.MIDDLE, EducationalStage.HIGH];
    ...
    const handleDeletePrincipalClick = (stage: EducationalStage, principal: Principal) => {
        if (window.confirm(t('confirmDeletePrincipal', { name: principal.name }))) {
            onDeletePrincipal(stage, principal.id);
        }
    };
    ...
    return (
        <div className="bg-gray-100 dark:bg-gray-900 p-6 rounded-2xl shadow-xl border-t-8 border-gray-700 dark:border-gray-600 w-full space-y-6 relative">
            ...
            <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100 text-center">{school.name}</h1>
                    ...
                    {allStages.map(stage => (
                        <div key={stage} className={`p-3 rounded-lg flex items-center justify-center transition-all ${school.stages.includes(stage) ? 'bg-blue-100 dark:bg-blue-900/50' : 'bg-gray-100 dark:bg-gray-700'}`}>
                             <label className="flex items-center gap-3 cursor-pointer">
                                ...
                                <span className="font-semibold text-gray-700 dark:text-gray-200">{t(`${stage.toLowerCase()}Stage` as any)}</span>
                            </label>
                        </div>
                    ))}
                 </div>
                ...
                <div className="space-y-6 max-h-[40vh] overflow-y-auto p-1">
                    ...
                    <div>
                        ...
                        <div className="space-y-3 p-2">
                             {principalFeatures.map(({ key, page }) => (
                                <FeatureToggle key={key} feature={key}>
                                    <div className="flex gap-1">
                                        {school.stages.map(stage => (
                                            <button
                                                key={stage}
                                                onClick={() => onEnterFeaturePage(page, stage)}
                                                disabled={school.featureFlags[key] === false}
                                                className="bg-blue-500 text-white font-semibold py-1 px-2 rounded-lg hover:bg-blue-600 transition shadow-sm disabled:bg-gray-400 disabled:cursor-not-allowed text-xs"
                                                title={`${t('manage')} - ${t(`${stage.toLowerCase()}Stage` as any)}`}
                                            >
                                                {t(`${stage.toLowerCase()}Stage` as any).substring(0, 1)}
                                            </button>
                                        ))}
                                    </div>
                                </FeatureToggle>
