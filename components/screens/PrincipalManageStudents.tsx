...
const PrincipalManageStudents: React.FC<PrincipalManageStudentsProps> = ({ stage, students, onAddStudent, onAddMultipleStudents, onUpdateStudent, onDeleteStudent, onBack, onLogout, toggleDarkMode, isDarkMode }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl border-t-8 border-blue-600 dark:border-blue-500 w-full relative">
            ...
            <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6 text-center">{t('manageStudents')}</h1>
             ...
             <div className="mb-8 p-4 bg-blue-50 dark:bg-gray-700/50 rounded-lg shadow-inner space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                     <select value={importLevel} onChange={e => setImportLevel(e.target.value)} className="w-full p-3 border-2 dark:border-gray-600 dark:bg-gray-800 rounded-lg bg-white">
                        {stageLevels.map(l => <option key={l} value={l}>{l}</option>)}
                    </select>
                    <select value={importClass} onChange={e => setImportClass(e.target.value)} className="w-full p-3 border-2 dark:border-gray-600 dark:bg-gray-800 rounded-lg bg-white">
            ...
            <form onSubmit={handleAdd} className="mb-8 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg shadow-inner space-y-4">
                ...
                <input type="text" placeholder={t('studentName')} value={name} onChange={e => setName(e.target.value)} className="w-full p-3 border-2 dark:border-gray-600 dark:bg-gray-800 rounded-lg" required />
                <input type="text" placeholder={t('guardianCode')} value={guardianCode} onChange={e => setGuardianCode(e.target.value)} className="w-full p-3 border-2 dark:border-gray-600 dark:bg-gray-800 rounded-lg" required />
                
                <div className="grid grid-cols-2 gap-4">
                    <select value={level} onChange={e => setLevel(e.target.value)} className="w-full p-3 border-2 dark:border-gray-600 dark:bg-gray-800 rounded-lg bg-white">
                        {stageLevels.map(l => <option key={l} value={l}>{l}</option>)}
                    </select>
                    <select value={studentClass} onChange={e => setStudentClass(e.target.value)} className="w-full p-3 border-2 dark:border-gray-600 dark:bg-gray-800 rounded-lg bg-white">
                        {CLASSES.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                </div>
            ...
            <div>
                ...
                <div className="max-h-60 overflow-y-auto space-y-2 p-2">
                    {students.length > 0 ? students.map(student => (
                        <div key={student.id} className="bg-white dark:bg-gray-800/50 p-3 rounded-lg shadow-sm flex justify-between items-center">
                            <div>
                                <p className="font-bold dark:text-gray-100">{student.name}</p>
                                <p className="text-sm text-gray-600 dark:text-gray-400">{student.level} - {student.class}</p>
                            </div>
                            <div className="flex gap-2">
                                <button onClick={() => setEditingStudent(student)} className="text-blue-600 hover:text-blue-800 font-bold px-3 py-1 text-sm">{t('edit')}</button>
                                <button onClick={() => handleDelete(student.id, student.name)} className="text-red-500 hover:text-red-700 font-bold px-3 py-1 text-sm">{t('delete')}</button>
                            </div>
                        </div>
                    )) : (
