...
interface TeacherAddQuizProps {
    quizzes: Quiz[];
    onSave: (quiz: Omit<Quiz, 'id'|'level'|'class'|'subject'|'date'>) => void;
...
const TeacherAddQuiz: React.FC<TeacherAddQuizProps> = ({ quizzes, onSave, onDelete, onBack, onLogout }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-xl border-t-8 border-blue-600 w-full relative">
            ...
            <div className="mb-8">
                <h2 className="text-xl font-semibold text-gray-700 mb-3 border-b-2 pb-2">{t('quizzes')}</h2>
                <div className="max-h-48 overflow-y-auto space-y-2 p-2 bg-gray-100 rounded-lg">
                    {quizzes.length > 0 ? (
                        quizzes.sort((a,b) => b.id - a.id).map(quiz => (
                            <div key={quiz.id} className="flex items-center justify-between bg-white p-3 rounded-lg shadow-sm">
                                <span className="text-gray-800 font-medium">{quiz.title}</span>
                                <button onClick={() => handleDeleteClick(quiz.id)} className="text-red-500 hover:text-red-700 font-bold px-3 py-1 rounded-md hover:bg-red-100 transition">
                                    {t('delete')}
                                </button>
                            </div>
                        ))
                    ) : (
                        <p className="text-gray-500 text-center py-4">{t('noQuizzes')}</p>
                    )}
                </div>
            </div>

            <div className="p-4 bg-gray-50 rounded-lg shadow-inner space-y-4 border-t-2 border-dashed pt-6">
                <h2 className="text-xl font-semibold text-gray-700 text-center">{t('addQuiz')}</h2>
                <p className="text-sm text-center text-gray-600">{t('uploadImageForQuiz')}</p>
                
                {imagePreview && (
                    <div className="p-2 border-2 border-dashed rounded-lg"><img src={imagePreview} alt="Preview" className="max-h-48 mx-auto rounded"/></div>
                )}
            ...
            {generatedQuiz && (
                <div className="my-6 p-4 bg-blue-50 rounded-lg shadow-inner space-y-4 animate-fade-in">
                    <h3 className="text-xl font-bold text-blue-800 text-center">{generatedQuiz.title}</h3>
                    <div className="space-y-4 max-h-60 overflow-y-auto p-2">
                        {generatedQuiz.questions.map((q, qIndex) => (
                            <div key={qIndex} className="bg-white p-3 rounded">
                                <p className="font-semibold">{qIndex + 1}. {q.question}</p>
                                <ul className="mt-2 space-y-1">
                                    {q.options.map((opt, oIndex) => (
                                        <li key={oIndex} className={`p-1 rounded ${oIndex === q.correctAnswerIndex ? 'bg-green-200 font-bold' : ''}`}>
                                            {opt}
                                        </li>
                                    ))}
                                </ul>
