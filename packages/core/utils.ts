...
export const snakeToCamelCase = (o: any): any => {
    if (isObject(o)) {
        const n: { [key: string]: any } = {};
        Object.keys(o).forEach((k) => {
            const newKey = k.replace(/_([a-z])/g, (g) => g[1].toUpperCase());
            n[newKey] = snakeToCamelCase(o[k]);
        });
        return n;
    } else if (Array.isArray(o)) {
        return o.map((i) => snakeToCamelCase(i));
    }
    return o;
};

export const camelToSnakeCase = (o: any): any => {
    if (isObject(o)) {
        const n: { [key: string]: any } = {};
        Object.keys(o).forEach((k) => {
            const newKey = k.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`);
            n[newKey] = camelToSnakeCase(o[k]);
        });
        return n;
    } else if (Array.isArray(o)) {
        return o.map((i) => camelToSnakeCase(i));
    }
    return o;
};
