...
const translations = {
  ar: {
    ...
    invalidCode: "الرمز المدخل غير صحيح. إذا كنت تحاول الدخول بحساب جديد لأول مرة، فالسبب على الأغلب هو إعدادات الأمان (RLS). الرجاء مراجعة لوحة تحكم المدير الخارق للحصول على التعليمات.",
    ...
    supabaseEmailConfirmationError: "<strong>فشل الدخول:</strong> خاصية تأكيد البريد الإلكتروني مفعلة.<br><br><strong>لحل المشكلة:</strong><br>1. اذهب إلى لوحة تحكم Supabase.<br>2. اختر 'Authentication' ثم 'Settings'.<br>3. قم <strong>بتعطيل</strong> خيار 'Confirm email'.<br>4. اذهب إلى 'Users'، واحذف هذا المستخدم.<br>5. حاول تسجيل الدخول مرة أخرى.",
    supabaseSignupsDisabledError: "<strong>فشل الدخول:</strong> تسجيل المستخدمين الجدد معطل.<br><br><strong>لحل المشكلة:</strong><br>1. اذهب إلى لوحة تحكم Supabase.<br>2. اختر 'Authentication' ثم 'Settings'.<br>3. قم <strong>بتفعيل</strong> خيار 'Allow new users to sign up'.<br>4. حاول تسجيل الدخول مرة أخرى.",
    ...
    rlsLoginError: "<strong>فشل الدخول:</strong> مشكلة في إعدادات الأمان.<br><br>هذا الخطأ يعني أن التطبيق لا يملك صلاحية البحث عن رمز الدخول للمستخدمين الجدد.<br><br><strong>لحل المشكلة:</strong><br>الرجاء الذهاب إلى <strong>Database &gt; Policies</strong> في Supabase، والتأكد من وجود سياسة تسمح بالوصول للقراءة (SELECT) للدور 'anon' على جداول <code>principals</code>, <code>teachers</code>, و <code>students</code>.",
  },
  fr: {
    ...
    invalidCode: "Code invalide. Si vous essayez de vous connecter avec un nouveau compte, il s'agit probablement d'un problème de configuration de la sécurité (RLS). Veuillez consulter le tableau de bord Super Admin pour obtenir des instructions.",
    ...
    statisticsDashboard_feature: "Statistiques",
    educationalTips_feature: "Conseils pédagogiques",
    ...
    supabaseEmailConfirmationError: "<strong>Connexion échouée :</strong> La confirmation par e-mail est activée.<br><br><strong>Pour corriger :</strong><br>1. Allez sur votre tableau de bord Supabase.<br>2. Allez à 'Authentication' puis 'Settings'.<br>3. Désactivez l'option 'Confirm email'.<br>4. Allez à 'Users', et supprimez cet utilisateur.<br>5. Réessayez de vous connecter.",
    supabaseSignupsDisabledError: "<strong>Connexion échouée :</strong> L'inscription de nouveaux utilisateurs est désactivée.<br><br><strong>Pour corriger :</strong><br>1. Allez sur votre tableau de bord Supabase.<br>2. Allez à 'Authentication' puis 'Settings'.<br>3. <strong>Activez</strong> l'option 'Allow new users to sign up'.<br>4. Réessayez de vous connecter.",
    rlsNoticeTitle: "Paramètre de sécurité important : Erreur 'Code invalide'",
    rlsNoticeBody1: "Pour autoriser les nouveaux utilisateurs (directeurs, enseignants, parents) à se connecter pour la première fois, une politique de sécurité au niveau des lignes (RLS) doit être ajoutée pour permettre un accès en lecture anonyme.",
    rlsNoticeBody2: "Sans cette politique, l'application ne peut pas trouver les nouveaux codes d'accès, ce qui entraîne l'apparition permanente de l'erreur 'Code invalide'.",
    rlsNoticeBody3: "Veuillez vous rendre dans <strong>Database -&gt; Policies</strong> dans Supabase et ajouter une nouvelle politique pour chacune des tables <code>principals</code>, <code>teachers</code>, et <code>students</code> qui autorise l'opération <strong>SELECT</strong> pour le rôle <strong>anon</strong>.",
    rlsLoginError: "<strong>Connexion échouée :</strong> Problème de paramètre de sécurité.<br><br>Cette erreur signifie que l'application n'est pas autorisée à rechercher le code d'accès pour les nouveaux utilisateurs.<br><br><strong>Pour corriger :</strong><br>Veuillez vous rendre sur <strong>Database &gt; Policies</strong> dans Supabase et vous assurer qu'une politique autorisant l'accès en lecture (SELECT) pour le rôle 'anon' existe pour les tables <code>principals</code>, <code>teachers</code>, et <code>students</code>.",
  },
  en: {
    // ...
  }
