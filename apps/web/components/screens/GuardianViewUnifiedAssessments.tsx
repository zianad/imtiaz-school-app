...
import { UnifiedAssessment, School, Student, Subject } from '../../../../packages/core/types';
...
const GuardianViewUnifiedAssessments: React.FC<GuardianViewUnifiedAssessmentsProps> = ({ school, student, subject, onBack, onLogout, toggleDarkMode, isDarkMode }) => {
    ...
    useEffect(() => {
        const fetchAssessments = async () => {
            ...
            else setAssessments(snakeToCamelCase(data));
            setIsLoading(false);
        };
        fetchAssessments();
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl border-t-8 border-blue-600 dark:border-blue-500 w-full relative">
            ...
            <div className="max-h-[70vh] overflow-y-auto space-y-3 p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                {isLoading ? <p>{t('loading')}...</p> : sortedAssessments.length > 0 ? (
