...
const TeacherExamProgramForm: React.FC<TeacherExamProgramFormProps> = ({ school, toggleDarkMode, isDarkMode, examPrograms, onSave, onDelete, onBack, onLogout }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl border-t-8 border-blue-600 dark:border-blue-500 w-full relative">
            ...
            <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6 text-center">{t('examProgram')}</h1>

            <div className="mb-8 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg shadow-inner space-y-4">
                <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-200 text-center">إضافة برنامج فروض جديد</h2>
            ...
            <div>
                <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-3 text-center border-t dark:border-gray-600 pt-4">البرامج الحالية</h2>
                <div className="max-h-60 overflow-y-auto space-y-2 p-2">
                    {examPrograms.length > 0 ? (
                        [...examPrograms].sort((a,b) => b.id - a.id).map(prog => (
                            <div key={prog.id} className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm flex justify-between items-center">
                                <p className="text-gray-700 dark:text-gray-300">
                                    برنامج - {new Date(prog.date).toLocaleDateString()}
                                </p>
                                <button onClick={() => onDelete(prog.id)} className="text-red-500 hover:text-red-700 font-bold px-3 py-1 text-sm">
                                    {t('delete')}
                                </button>
                            </div>
                        ))
                    ) : (
                        <p className="text-center text-gray-500 dark:text-gray-400 py-4">لا توجد برامج حاليا.</p>
                    )}
                </div>
            </div>
