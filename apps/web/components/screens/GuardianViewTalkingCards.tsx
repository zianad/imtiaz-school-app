...
import { TalkingCard, School, Student } from '../../../../packages/core/types';
...
const GuardianViewTalkingCards: React.FC<GuardianViewTalkingCardsProps> = ({ school, student, onBack, onLogout, toggleDarkMode, isDarkMode }) => {
    ...
    useEffect(() => {
        const fetchCards = async () => {
            ...
            else setCards(snakeToCamelCase(data));
            setIsLoading(false);
        };
        fetchCards();
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl border-t-8 border-purple-600 dark:border-purple-500 w-full relative">
            ...
            <div className="max-h-[70vh] overflow-y-auto grid grid-cols-2 md:grid-cols-3 gap-4 p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                {isLoading ? <p className="col-span-full text-center">{t('loading')}...</p> : sortedCards.length > 0 ? (
