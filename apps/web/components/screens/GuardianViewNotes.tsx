...
import { Note, Absence, Student, School } from '../../../../packages/core/types';
...
const GuardianViewNotes: React.FC<GuardianViewNotesProps> = ({ student, school, notes, absences, onBack, onLogout, toggleDarkMode, isDarkMode }) => {
    const { t } = useTranslation();

    const studentNotes = notes
        .filter(n => n.studentIds.includes(student.id) && n.status === 'approved')
                ...
                <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                    <h2 className="text-xl font-bold text-gray-700 dark:text-gray-200 text-center mb-3">الغيابات</h2>
                     <div className="space-y-2">
                        {studentAbsences.length > 0 ? studentAbsences.map(absence => (
