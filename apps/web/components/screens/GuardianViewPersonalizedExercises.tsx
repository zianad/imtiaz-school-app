...
import { PersonalizedExercise, Subject, School } from '../../../../packages/core/types';
...
const GuardianViewPersonalizedExercises: React.FC<GuardianViewPersonalizedExercisesProps> = ({ school, exercises, onBack, onLogout, toggleDarkMode, isDarkMode }) => {
    const { t } = useTranslation();
    const sortedExercises = [...exercises].sort((a,b) => b.date.getTime() - a.date.getTime());
    
    const isArabicContent = exercises.length > 0 && exercises[0]?.subject === Subject.Arabic;
