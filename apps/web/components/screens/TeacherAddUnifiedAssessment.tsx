...
interface TeacherAddUnifiedAssessmentProps {
    ...
    onSave: (data: { title: string; image?: string; pdf?: { name: string; url: string } }) => void;
...
const TeacherAddUnifiedAssessment: React.FC<TeacherAddUnifiedAssessmentProps> = ({ school, toggleDarkMode, isDarkMode, assessments, onSave, onDelete, onBack, onLogout }) => {
    ...
    const handleSave = async () => {
        ...
        let data: { title: string; image?: string; pdf?: { name: string; url: string } } = { title };
            ...
            <div>
                ...
                <div className="max-h-60 overflow-y-auto space-y-2 p-2">
                    {assessments.length > 0 ? (
                         [...assessments].sort((a,b) => b.date.getTime() - a.date.getTime()).map(item => (
                            <div key={item.id} className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm flex justify-between items-center">
                                <p className="text-gray-700 dark:text-gray-300 truncate">{item.title}</p>
                                <button onClick={() => onDelete(item.id)} className="text-red-500 hover:text-red-700 font-bold px-3 py-1 text-sm">
                                    {t('delete')}
                                </button>
                            </div>
                        ))
